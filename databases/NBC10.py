...
# Longstanding revision in most benchmarking papers
BIND_NBC10A = {}
...
BINDINFO_NBC100 = {}
for rxn in HRXN:
    # alternately, all collected into NBC10-0 in 'citation': 'dft'
    if (rxn in BzBz_S) or (rxn in BzBz_T) or (rxn in BzBz_PD34) or \
       (rxn in BzBz_PD32) or (rxn in BzBz_PD36):
        BINDINFO_NBC100['%s-%s' % (dbse, rxn)] = {'citation': 'bzbzcurve', 'method': 'CCSDT', 'mode': 'CP', 'basis': 'atqzhadtz'}  # CBS(aQZ; H:aTZ-aQZ; $\Delta$:H:haDZ-haTZ)
    elif rxn in PyPy_S2:
        BINDINFO_NBC100['%s-%s' % (dbse, rxn)] = {'citation': 'bzpypypy', 'method': 'CCSDT', 'mode': 'CP', 'basis': 'atqzhadtz'}   # CBS(aQZ; H:aTZ-aQZ; $\Delta$:H:haDZ-haTZ)
    elif (rxn in BzH2S) or (rxn in BzMe):
        BINDINFO_NBC100['%s-%s' % (dbse, rxn)] = {'citation': 'bzbzcurve', 'method': 'CCSDT', 'mode': 'CP', 'basis': 'atqzadtz'}   # CBS(aQZ; H:aTZ-aQZ; $\Delta$:H:aDZ-aTZ)
    elif rxn in MeMe:
        BINDINFO_NBC100['%s-%s' % (dbse, rxn)] = {'citation': 'scsmp2nci', 'method': 'CCSDT', 'mode': 'CP', 'basis': 'atqz'}       # CBS(aQZ; -- ; H:aTZ-aQZ)
    elif rxn in PyPy_T3:
        BINDINFO_NBC100['%s-%s' % (dbse, rxn)] = {'citation': 'bzpypypy', 'method': 'CCSDT', 'mode': 'CP', 'basis': 'atqzadz'}     # CBS(aQZ; H:aTZ-aQZ; $\Delta$:aDZ)
BINDINFO_NBC10A = {}
for rxn in HRXN:
    if (rxn in BzBz_S) or (rxn in BzBz_T) or (rxn in BzBz_PD34) or \
...
DATA['SAPT DISP ENERGY']['NBC1-BzBz_S-4.1'   ] =   -3.6034
